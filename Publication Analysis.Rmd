...
y1 = ggplot(arl_cornyld_soilcld, aes(x = soil, y = yield, 
  ...
  geom_text(aes(label = .group), nudge_y = 3100, size=7) +
  geom_jitter(data = corn %>% 
                filter(location == "Arlington"), mapping = aes(y = yield), alpha = 0.2, size= 3) +
  #coord_flip() +
...
```{r}
y2 = ggplot(lan_cornyld_soilcld, aes(x = soil, y = yield, color = soil)) +
  geom_point(size= 5) +
  geom_errorbar(aes(ymin= lower.CL, ymax= upper.CL), width= .3, size= 1) +
  geom_text(aes(label = .group), nudge_y = 3300, size=7) +
  geom_jitter(data = corn %>% 
                filter(location == "Lancaster"), mapping = aes(y = yield), alpha = 0.2, size= 3) +
...
### Corn Yield Figure with 1 X-axis label

```{r, fig.width= 12, fig.height= 6}
CNyieldcld= rbind(arl_cornyld_soilcld, lan_cornyld_soilcld)

y9 = ggplot(CNyieldcld, aes(x = soil, y = yield, color = soil)) +
  geom_point(size= 5) +
  geom_errorbar(aes(ymin= lower.CL, ymax= upper.CL), width= .3, size= 1) +
  geom_text(aes(label = .group), nudge_y = 3300, size=7) +
...
y3 <- ggplot(filter(soyyld_soilcld, location == "Arlington"), aes(x = soil, y = yield, 
  ...
  geom_point(size= 5) +
  geom_errorbar(aes(ymin= lower.CL, ymax= upper.CL), width= .3, size= 1) +
  #geom_text(aes(label = .group), nudge_y = 800, size=7) +
...
SByield<- {y3 + y4}+
  ...
  theme = theme(plot.title = element_text(size = 30),
                plot.subtitle= element_text(size = 20))) 
  ggsave("Soy_YieldFigureSI.jpeg", height= 6, width= 12)#Need to alter element text
SByield
```
