...
```{r}
soy_weedemmeans <- emmeans(soy_weed1, ~ soil*pre, contr="pairwise", adjust="tukey", type= "response")
# Consider using | instead * if you want to compare soil manag in each pre
soy_weedcld <- cld(soy_weedemmeans$emmeans, alpha=0.05, Letters=letters, adjust="none", sort=TRUE, reverse= TRUE)
...
w4 <- ggplot(soy_weedcld, aes(x = soil, y = weed_cover*100, color = soil)) +
  ...
  ylim(0,100)+
  labs(title= "Soybean Weed Ground Cover - Tukey",x= "Soil Management Practice", y= "% Weed Ground Cover")
w4
